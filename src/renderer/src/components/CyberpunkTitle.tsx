...
const CyberpunkTitle: React.FC<CyberpunkTitleProps> = ({ text, className = '' }) => {
  ...
  return (
    <div className="relative">
      ...
      >
        {text}
        <span className="absolute top-0 left-0 w-full h-full text-red-500/20 animate-glitch-2">{text}</span>
        <span className="absolute top-0 left-0 w-full h-full text-red-500/20 animate-glitch-3">{text}</span>
      </h1>
