...
interface ExpenseState {
  ...
  addCashFlowTransaction: (transation: Omit<CashFlowTransaction, 'id'>) => void
  setCashFlowTransaction: (transtion: CashFlowTransaction[]) => void
}
...
export const useExpenseStore = create<ExpenseState>()((set, get) => ({
  ...
  addSpendingGoal: (goal) =>
    set((state) => ({
      spendingGoals: [...state.spendingGoals, goal]
    })),
  removeSpendingGoal: (category) =>
    set((state) => ({
  ...
  addBillSchedule: (bill) =>
    set((state) => ({
      billSchedules: [...state.billSchedules, bill]
    })),
  updateBillSchedule: (id, updates) =>
    set((state) => ({
  ...
  cashFlowTransaction: (() => {
    ...
    try {
      const storeTransactions = window.localStorage.getItem('cashFlowTransactions')
      if (!storeTransactions) return []

      const parseTranscations = JSON.parse(storeTransactions)
      return Array.isArray(parseTranscations) ? parseTranscations : []
    } catch (error) {
      console.error('Failed to parse CashFlowTransactions from localstorage', error)
      return []
    }
  })(),
  setCashFlowTransaction: (transactions) => {
    if (!Array.isArray(transactions)) {
      console.error('Invalid Transaction data type')
      return
    }

    try {
      window.localStorage.setItem('cashFlowTransactions', JSON.stringify(transactions))
      set({ cashFlowTransaction: transactions })
    } catch (error) {
      console.error('Failed to save Transactions', error)
    }
  },
  addCashFlowTransaction: (transactions) => {
    set((state) => {
      ...
      try {
        ...
        const updateTransactions = [...state.cashFlowTransaction, newTranscation]
        window.localStorage.setItem('cashFlowTransactions', JSON.stringify(updateTransactions))
        return { cashFlowTransaction: updateTransactions }
      } catch (error) {
